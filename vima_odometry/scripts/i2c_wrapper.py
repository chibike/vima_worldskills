...
class I2cObject(object):
    ...
    def read_as_sint(self, register, howmany):
        ...
        hex_string = '0x'
        for i in data:
            hex_string = hex_string + self.__ensure_length(hex(i)[2:], 2, '0', '')
    ...
    def concatenate(self, data):
        ...
        if isinstance(data, list) or isinstance(data, tuple):
            temp_str = "0x"
            for i in data:
                temp_str = temp_str + self.__ensure_length(hex(i)[2:], 2, '0', '')
            return eval(temp_str)
        else:
            return data

    def __ensure_length(self, string, length=2, left_padding='0', right_padding=''):
