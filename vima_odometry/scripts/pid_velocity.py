...
class PidVelocity():
    def __init__(self):
        ...
        ### update pid values ####
        self.update_pid_values(10, 10, 0.001)
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            self.spinOnce()
            self.update_pid_values(self.Kp, self.Ki, self.Kd)
            self.r.sleep()
...
#        rospy.logdebug("-D- %s wheelCallback msg.data= %0.3f wheel_latest = %0.3f mult=%0.3f" % (self.nodename, enc, self.wheel_latest, self.wheel_mult))
    ...
    def targetCallback(self, msg):
        self.target = msg.data*10.0
        self.ticks_since_target = 0
        # rospy.logdebug("-D- %s targetCallback " % (self.nodename))

    def update_pid_values(self, p, i, d):
        self.Kp = rospy.get_param('~Kp',p)
        self.Ki = rospy.get_param('~Ki',i)
        self.Kd = rospy.get_param('~Kd',d)
