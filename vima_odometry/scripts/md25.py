...
class MD25(object):
    ...
    def get_mode(self):
        ...
        new_mode = 255
        while new_mode > 3:
            new_mode = self.i2c_object.read_as_uint(15, 1)
            if new_mode > 3:
    ...
    def set_wheel_speed_right(self, speed):
        '''Sets the speed of the right wheel'''
        self.get_mode()

        self.right_speed = generic_functions.constrainf(speed + 1, -100, 100)
        ...
        if (self.mode == 0) or (self.mode == 1):
            ...
            value = self.__translate_value(self.right_speed, -100, 100)
            self.i2c_object.write(0, value)
        else:
            raise ValueError("Invalid mode({0}) detected".format(self.mode))
    ...
    def get_encoder_counts(self):
        ...
        displacement_diff_left  = 0
        displacement_diff_right = 0
        ...
        for i in range(10):
            ...
            if abs(displacement_diff_left) > 300 or abs(displacement_diff_right) > 300:
                encoder_counts["left_count"]  = self.i2c_object.read_as_sint(2, 4)
                encoder_counts["right_count"] = self.i2c_object.read_as_sint(6, 4)
            else:
                break
...
def run_overall_test_routine():
    ...
    try:
        ...
        print "Has Value Error",e
        return
    except IOError as e:
        print "Has IO Error",e
        return
