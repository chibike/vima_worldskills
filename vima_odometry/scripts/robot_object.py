...
class RobotObject(object):
	def __init__(self, robot_name="vima_worldskills", start_pin=4, wheel_diameter=0.1, distance_btw_wheels=0.265, encoder_counts_per_rev=360.0):
		...
		# Setup external start button callback
		self.external_start_button_callback = None
	...
	def set_external_start_button_callback(self, function=None):
		self.external_start_button_callback = function
	...
	def get_start_switch_state(self):
		...
		#print self.start_btn_state
		return self.start_btn_state

	def start_btn_callback(self, data):
		self.get_start_switch_state()
		print data," := =: ", type(data)
		if self.external_start_button_callback != None:
			self.external_start_button_callback()
			try:
				#self.external_start_button_callback()
				pass
			except Exception as e:
	...
	def move_distance(self, target_displacement=4.0, speed=1.0):
		...
		if target_displacement == 0:
			self.wheels_controller.stop_wheels()
			return
	...
	def rotate(self, angle=math.pi, speed=1.0):
		# reset wheel data
		self.__reset_wheel_data()
		...
		target_wheel_displacement = angle * (self.distance_btw_wheels/2.0)

		# Start moving
		sign = target_wheel_displacement/abs(target_wheel_displacement)
		speed = sign * abs(speed)
		self.set_wheel_speeds(speed, -speed)

		current_displacement = 0.0
		while current_displacement < abs(target_wheel_displacement):
			self.__update_wheel_data()
			current_displacement = abs(max(abs(self.curr_left_displacement), abs(self.curr_right_displacement)))
			print "Dist: {0}m    Wheels: ({3}, {4})m     Speed: ({1}, {2})m/s".format(current_displacement, self.left_wheel_speed, self.right_wheel_speed, self.curr_left_displacement, self.curr_right_displacement)
...
if __name__ == "__main__":
	...
	def test_odometry():
		...
		my_robot.move_distance(5.0, 0.5)

		print "turningn left"
		my_robot.rotate(-math.pi, 0.5)

		print "moving forward"
		my_robot.move_distance(1.0, 0.5)
	...
	while True:
		if my_robot.get_start_switch_state():
			test_odometry()
			break